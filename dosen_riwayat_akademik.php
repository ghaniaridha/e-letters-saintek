...
$where_clause = "
    ...
    (
        ...
        OR
        (dsr.id_pb1 = '$id_dosen' AND dsr.status_pb1 != 'Menunggu')
        OR
...
$query = mysqli_query($koneksi, "
    SELECT 
        ...
        COALESCE(
            CASE 
                WHEN dsr.id_pb1 = '$id_dosen' THEN dsr.waktu_pb1
                WHEN dsr.id_pb2 = '$id_dosen' THEN dsr.waktu_pb2
                WHEN dak.id_pa = '$id_dosen' THEN dak.waktu_pa
            END, 
            sp.tanggal_pengajuan
        ) AS waktu_tindak_lanjut
    FROM surat_pengajuan sp
    JOIN mahasiswa m ON sp.id_mhs = m.id_mhs
...
<head>
    ...
    <title>Riwayat Verifikasi Akademik</title>
...
<body class="dosen-page">
    ...
    <section id="daftar-surat" class="daftar-surat">
        <div class="riwayat-permohonan-header">
            <h2>Riwayat Verifikasi Akademik</h2>
        </div>
                ...
                <tbody>
                    <?php if ($query && mysqli_num_rows($query) > 0) {
                        ...
                        while ($row = mysqli_fetch_assoc($query)) {
                            ...
                            if (empty(trim($statusAnda))) {
                                if (strtolower($row['status_akhir']) == 'perbaikan') {
                                    $statusAnda = 'Menunggu Perbaikan';
                                } else {
                                    $statusAnda = '-';
                                }
                            }

                            $status_cek = strtolower($statusAnda);
                            if (strpos($status_cek, 'ditolak') !== false) {
                                $badge_class = 'status-ditolak';
                            } elseif (strpos($status_cek, 'disetujui') !== false) {
                                $badge_class = 'status-selesai';
                            } elseif (strpos($status_cek, 'perbaikan') !== false) {
                    ...
                    ?>
                            <tr>
                                ...
                                <td>
                                    <?php if ($statusAnda !== '-' && !empty($statusAnda)): ?>
                                        <span class="badge-status <?= $badge_class; ?>">
                                            <?= htmlspecialchars($statusAnda); ?>
                                        </span>
                                    <?php else: ?>
                                        <?= htmlspecialchars($statusAnda); ?>
                                    <?php endif; ?>
                                ...
                                <td>
                                    <a href="dosen_detail_permohonan.php?id=<?= $row['id_surat']; ?>&asal=riwayat_akademik" class="btn-aksi">Detail</a>
                                </td>
                            </tr>
                        <?php }
                    } else { ?>
                        <tr>
                            <td colspan="8" class="empty-table-cell">
                                ...
                                <p>Belum ada riwayat verifikasi akademik yang sesuai dengan filter.</p>
                            </td>
                        </tr>
                    <?php } ?>
                </tbody>
