...
$q_jenis = mysqli_query($koneksi, "SELECT * FROM jenis_surat WHERE id_jenis IN (11, 12) ORDER BY nama_surat ASC");
?>
        ...
        <div class="table-wrapper" id="template-surat">
            <form method="GET" action="" class="filter-container">
                <div class="search-container">
                    ...
                    <input type="text" name="search" class="search-input" placeholder="Cari Nama Organisasi / Ormawa..." value="<?= htmlspecialchars($search); ?>">
                </div>
                ...
                <tbody>
                    <?php if ($query && mysqli_num_rows($query) > 0) {
                        ...
                        while ($row = mysqli_fetch_assoc($query)) {
                            $status_tampil = $row['status_akhir'];
                            $jenis_org = $row['jenis_organisasi'] ?? 'Ormawa';

                            if (strpos(strtolower($status_tampil), 'menunggu') !== false) {
                                if ($jenis_org == 'Ormawa') {
                                    $status_tampil = 'Menunggu Persetujuan Kaprodi';
                                } else {
                                    $status_tampil = 'Menunggu Persetujuan Pembina';
                                }
                            }
                    ?>
                            <tr>
                        ...
                        <tr>
                            <td colspan="6" class="empty-table-cell">
                                ...
                                <p>Tidak ada permohonan organisasi yang sesuai dengan filter.</p>
                            </td>
                        </tr>
                    <?php } ?>
                </tbody>
